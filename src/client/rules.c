...
BoardState *init_bs(const int player_color) {
  ...
  bs->black_captured_stones = 0;
  bs->white_captured_stones = 0;
...
int move(BoardState *bs, char *return_buf, int buf_len) {
  ...
  if (bs->color == BLACK)
    bs->black_captured_stones += captured_stone_count;
  else
    bs->white_captured_stones += captured_stone_count;
